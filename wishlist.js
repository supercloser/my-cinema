...
const savedMovies = JSON.parse(localStorage.getItem("wishlist")) || [];
...
function removeFromWishlist(movieID) {
  ...
  for (const movie of savedMovies) {
    if (movie.movieID === removeMovieID) {
      ...
      localStorage.setItem("wishlist", JSON.stringify(savedMovies));
  ...
  for (const movieObject of movies) {
    const template = `
        <div class="result">
            ...
            <button class="wishlist" title="Remove from wishlist" onclick="removeFromWishlist(${
              movieObject.movieID
            });"><img src="https://github.com/supercloser/my-cinema/blob/main/Assets/broken_heart.png?raw=true" alt="wishlist" class="broken-heart"></button>
